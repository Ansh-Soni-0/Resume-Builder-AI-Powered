...
const TemplateSelector = ( { selectedTamplate , onChange}) => {
    const [isOpen , setIsOpen] = useState(false);
    const templates = [
        {
            id:"classic",
            name:"Classic",
            preview:"A clean, traditional resume format with clear sections and professinal typography"
        },
        {
            id: "modern",
            name: "Modern",
    ...
    //when click outside the color picker box color picker close
            ...
            isOpen && (
                <div className='absolute top-full w-xs p-3 mt-2 space-y-3 z-10 bg-white rounded-md border border-gray-200 shadow-sm'>
                    {templates.map((template) => (
                        ...
                        className={`relative p-3 border rounded-md cursor-pointer transition-all ${selectedTamplate === template.id ? 'border-blue-400 bg-blue-100' : 'border-gray-300 hover:border-gray-400 hover:bg-gray-100'}`}

                        onClick={() => {onChange(template.id); setIsOpen(false)}} 
                        key={template.id}>
                            {
                                selectedTamplate === template.id && (
