...
const enhanceProfessinalSummary = async (req, res) => {
  try {
...
const enhanceJobDescription = async (req, res) => {
  try {
    ...
    const response = await ai.chat.completions.create({
      ...
      reasoning_effort: "low",
      messages: [
        { role: "system", content: "You are an expert in resume writing. Your task is to enhance the jobi description of a resume. The job description should be only in 1-2 sentence also highlighting key responsibilities and achievements. Use action verbs and quantifiable results where possible. Make it ATS-friendly. and only return text no options or anything else." },
        {
          role: "user",
          content: userContent,
        },
...
const uploadResume = async (req, res) => {
  try {
    ...
    {
        professinal_summary: { type: String, default: "" },
        skills: [{ type: String }],
        personal_info: {
